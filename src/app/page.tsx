...
import { 
  ...
  Zap, Search, Plus, Minus, Layers, LayoutPanelLeft,
...
export default function Home() {
  const { 
    ...
    subtotal, total, totalDays, discount, tax, setDiscount, setTax, 
    notes, setNotes, currency, setCurrency, currencySymbol 
  } = useQuoteStore();
  ...
  return (
    <div className="h-screen bg-[#FBFBFB] flex flex-col font-sans selection:bg-indigo-600 selection:text-white overflow-hidden">
      
      {/* 1. NAVBAR -*/}
      <nav className="h-16 border-b bg-white/80 backdrop-blur-md shrink-0 flex items-center justify-between px-8 z-50">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/*  */}
        <div className="flex-1 overflow-y-auto p-10 bg-grid-slate-100/50 custom-scrollbar">
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            {/* SMART TABS */}
            <div className="flex gap-2 p-1.5 bg-white rounded-2xl border shadow-sm sticky top-0 z-40">
              {CATEGORIES.map(t => (
                  ...
                  <Card key={s.id} 
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                       <div className="flex items-center gap-1 text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                         <Calendar size={12}/> {s.estimatedDays} Days
                       </div>
        ...
        <div className="w-[480px] bg-white border-l shadow-2xl flex flex-col z-20 h-full">
          {/* Header (Fixed) */}
                  ...
                  <AnimatePresence mode='popLayout'>
                    {selectedItems.map(i => (
                      <motion.div layout key={i.id} 
                        ...
                        className="p-4 bg-white rounded-2xl border-2 border-slate-50 shadow-sm space-y-4">
          ...
          {/* FINANCIALS (Pinned Bottom) */}
          <div className="p-10 bg-slate-950 text-white rounded-t-[40px] shadow-[0_-20px_60px_rgba(0,0,0,0.3)] space-y-8 shrink-0">
